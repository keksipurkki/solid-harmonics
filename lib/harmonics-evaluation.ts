...
type Grid3D = Vector3D[];

const idx = (l: number, m: number) => l * (l + 1) + m;

function cartiter(lmax: number) {
  const output = filter(([a, b, c]: Exponent3D) => a + b + c <= lmax, cartesian);
  const r = [...range(0, lmax + 1)];
  return output(r, r, r);
}

function lexicopos([a, b, c]: Exponent3D, lmax: number) {
 ...
 *
 */
function implementEvalAlgos(harmonics: any) {
