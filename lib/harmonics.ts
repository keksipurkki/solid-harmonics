...
function recurrenceMin(S: SphericalHarmonics, l: number) {
  const a = A(l);
...
function recurrenceMax(S: SphericalHarmonics, l: number) {
  const a = A(l);
...
function recurrenceMid(S: SphericalHarmonics, l: number, m: number) {
 ...
 *
 */
const proxyHandler: ProxyHandler<Polynomial3D[]> = {
  get(target, prop) {
...
class SphericalHarmonics extends Array {
  constructor(private lmax: number) {
    super(lmax + 1);
